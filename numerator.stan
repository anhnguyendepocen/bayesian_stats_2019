functions { /* saved as numerator.stan in R's working directory */
  vector numerator(vector pi, real M, real m, int N, int y) {
    ...
    vector[K] log1m_pi = log1m(pi); // log1m(x) == log(1 - x) but numerically accurate when its close to 0
    ...
    return exp(log_prior + log_likelihood); // equals exp(log_prior) * exp(log_likelihood), basically log prior pdf * pmf * pi which is the numerator?
  }
}
